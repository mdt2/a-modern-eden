...
var titleCase = _shared.titleCase;
...
var isPattern = function isPattern(obj) {
  return obj.hasOwnProperty('path');
}; /**
    * @module parse/patterns
    */

var collectionPath = function collectionPath(itms) {
  return path.dirname(path.normalize(itms[Object.keys(itms).pop()].path));
};
var collectionKey = function collectionKey(itms) {
  return collectionPath(itms).split(path.sep).pop();
};

function checkNamespaceCollision(key, obj, id) {
  var options = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};

  if (Array.isArray(key)) {
    return key.map(function (oneKey) {
      return checkNamespaceCollision(oneKey, obj, id, options);
    });
  }
...
function isHidden(collection, pattern, patternKey) {
  return collection.hidden && collection.hidden.indexOf(patternKey) !== -1 || pattern.data && pattern.data.hidden;
}
...
function isCollection(obj) {
  ...
  return Object.keys(obj).some(function (childKey) {
    return isPattern(obj[childKey]);
  });
}
 ...
 * @return {Boolean}
 */
var hasPatternOrdering = function hasPatternOrdering(itms) {
  return Object.keys(itms).some(function (itm) {
...
function buildPattern(patternObj, options) {
  var patternFile = { path: patternObj.path };
  return Object.assign(patternObj, {
    name: patternObj.data && patternObj.data.name || titleCase(resourceKey(patternFile))
  });
}
...
function buildPatterns(collectionObj, options) {
  var patterns = {};
  for (var childKey in collectionObj) {
...
function buildOrderedPatterns(collection) {
  var sortedKeys = void 0;
  if (hasPatternOrdering(collection.items)) {
    sortedKeys = Object.keys(collection.items).sort(function (keyA, keyB) {
      var orderA = collection.items[keyA].data.order || 10000;
      var orderB = collection.items[keyB].data.order || 10000;
  ...
  var patterns = [];
  sortedKeys = sortedKeys.concat(Object.keys(collection.items).filter(function (itemKey) {
...
function buildCollection(collectionObj, options) {
  var items = buildPatterns(collectionObj, options);
  var pseudoFile = { path: collectionPath(items) };
  return readFiles(collectionGlob(items), options).then(function (collData) {
    var collectionMeta = collData.length ? collData[0].contents : {};
    collectionObj.collection = Object.assign({
      name: titleCase(collectionKey(items)),
      resourceType: options.keys.collections.singular,
      id: resourceId(pseudoFile, options.src.patterns.basedir, options.keys.collections.plural)
    }, collectionMeta);
    checkNamespaceCollision(['items', 'patterns'], collectionObj.collection, 'Collection ' + collectionObj.collection.name, options);
    collectionObj.collection.items = items;
    collectionObj.collection.patterns = buildOrderedPatterns(collectionObj.collection);
 ...
 */
function buildCollections(patternObj, options) {
  var collectionPromises = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];
  ...
  for (var patternKey in patternObj) {
...
function parsePatterns(options) {
  return readFileTree(options.src.patterns, options.keys.patterns, options).then(function (patternObj) {
    return Promise.all(buildCollections(patternObj, options)).then(function () {
      return patternObj;
    }, function (error) {
